...
from onedatafilerestclient.types import SpaceSpecifier, FilePath, FileId

from .utils import split_onedata_path, normalize_onedata_path, validate_onedata_path
from .config import parse_onedata_url, get_onedata_config_from_env, merge_config
...
class OnedataFile(AbstractBufferedFile):
    ...
    def __init__(self, fs, path, mode="rb", block_size=None, cache_type="readahead", **kwargs):
...
class OnedataFileSystem(AbstractFileSystem):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not self.onezone_host or not self.token:
            raise ValueError("Both onezone_host and token must be provided either as parameters or environment variables")
    ...
    def ls(self, path: str, detail: bool = False, **kwargs) -> Union[List[str], List[Dict[str, Any]]]:
        ...
        if not space_name:
            ...
            if detail:
                return [{"name": space, "type": "directory", "size": 0} for space in spaces]
            else:
                return spaces
        ...
        try:
            ...
            for child in result["children"]:
                name = child["name"]
                full_path = f"{space_name}/{file_path}/{name}" if file_path else f"{space_name}/{name}"
    ...
    def info(self, path: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            attrs = self.client.get_attributes(
                space_name,
                file_path=file_path,
                attributes=["name", "type", "size", "mtime", "atime", "posixPermissions"]
            )
