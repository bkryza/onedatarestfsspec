...
from onedatafsspec.core import OnedataFileSystem, OnedataFile
...
def fs():
    ...
    with patch('onedatafsspec.core.OnedataFileRESTClient', return_value=client_mock), \
         patch('onedatafsspec.core.get_onedata_config_from_env', return_value={
            "onezone_host": None,
            "token": None, 
...
class TestOnedataFileSystem:
    ...
    def test_init_missing_params(self):
        ...
        with patch('onedatafsspec.core.get_onedata_config_from_env') as mock_env_config:
            ...
            with pytest.raises(ValueError, match="Both onezone_host and token must be provided"):
                OnedataFileSystem()
    ...
    def test_rm_file(self, fs):
        ...
        fs.client.remove.assert_called_once_with(
            "space1",
            file_path="test.txt"
        )
...
class TestOnedataFile:
    ...
    def test_init_read_mode(self, fs):
        ...
        with patch.object(fs, 'info', return_value={
            'name': 'space1/test.txt',
            'size': 100,
            'type': 'file'
        }), \
